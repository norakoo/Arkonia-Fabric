...
public class WindCallerEntity extends RaiderEntity implements GeoEntity {
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(5, new WindCallerEntity.CelebrateGoal(this));
    }

    class AttackGoal extends MeleeAttackGoal {
        public AttackGoal(WindCallerEntity windcaller) {
            super(windcaller, 1.0, false);
        }
    ...
    protected SoundEvent getAmbientSound() {
        if(this.polnareff) {
            return ArkoniaSounds.WINDCALLER_POLNAREFF;
        } else {
            return ArkoniaSounds.WINDCALLER_AMBIENT;
        }
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource source) {
        return ArkoniaSounds.WINDCALLER_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ArkoniaSounds.WINDCALLER_DEATH;
    }
    ...
    private PlayState attackPredicate(AnimationState<WindCallerEntity> windCallerEntityAnimationState) {
        if(this.handSwinging && windCallerEntityAnimationState.getController().getAnimationState().equals(AnimationController.State.STOPPED)) {
            windCallerEntityAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.windcaller.blast", Animation.LoopType.DEFAULT));
